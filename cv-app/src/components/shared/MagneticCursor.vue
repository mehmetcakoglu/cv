...
<template>
    <div class="fixed inset-0 pointer-events-none z-[999] no-print">
        <!-- Main Outer Cursor -->
        <div ref="cursor"
            class="fixed top-0 left-0 w-8 h-8 -ml-4 -mt-4 border border-white/20 rounded-full mix-blend-difference">
        </div>

        <!-- Exact Center Dot -->
        <div ref="cursorDot"
            class="fixed top-0 left-0 w-1 h-1 -ml-0.5 -mt-0.5 bg-white rounded-full mix-blend-difference"></div>

        <!-- Hover Text/Label if active -->
        <div v-if="isHovered && activeType === 'view'"
            class="fixed top-0 left-0 -translate-x-1/2 -translate-y-12 text-[10px] font-black uppercase tracking-[0.2em] text-white">
            View
        </div>
    </div>
</template>
